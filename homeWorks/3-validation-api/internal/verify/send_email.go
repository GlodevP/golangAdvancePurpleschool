...
func (handler *VerifyHandler) sendEmailVerify(emailAddr string) error {
	...
	err := mail.Send(handler.dependens.cfg.EmailConfig.EmailServer, smtp.PlainAuth("", handler.dependens.cfg.EmailConfig.EmailSendler, handler.dependens.cfg.EmailConfig.EmailSecret, handler.dependens.cfg.EmailConfig.EmailServer))
	if err != nil {
		fmt.Println(err)
		return err
	}
